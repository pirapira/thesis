...
今後はnpurlっぽい抽出を考えるべきではなかろうか。
それか、topologicalなんちゃらを頑張るべきか。
...
{\tiny
 ...
 \caption[Double negation elimination destroys the meaning of
 \fix{which} modalities]
{Adding a classical principle called
...
\AxiomC{I.H.}
 ...
 \caption[Parts of Proofs for Lemma~\ref{naka}.]
 {Parts of Proofs for Lemma~\ref{naka}. I.H. stands for induction hypothesis.}
 \label{wedge}
\end{sidewaysfigure}
...
In the library code, the worker threads only use non-blocking @MVar@
operations (a blocking operation @readMVar@ is used but by the main thread).
...
\begin{center}
\begin{center}
 \AxiomC{$\hyper_0\hmid@K t a@$}
 \AxiomC{$\hyper_1\hmid@K s b@$}
 \BinaryInfC{$\hyper_0\hmid\hyper_1\hmid @K t b@\hmid @K s a@$}
 \DisplayProof
\end{center}
\end{center}
...
Lindley~\fix{cite} provided a concise proof of strong normalization of
the natural deduction for 
intuitionistic propositional logic, containing disjunction.
Lindley's proof was formalized in Nominal Isabelle by \fix{whom}.
