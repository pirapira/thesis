...
\newcommand{\T}[1]{\LeftLabel{(T)}\UnaryInfC{${#1}$}}
...
\section{The Reason for Another Logic}
...
\section{Application to Wait-free Communication}
...
The rest of the paper is mostly in the bottom-to-top order.
In Chapter~\ref{iec}, we define and examine the syntax and semantics of
intuitionistic epistemic logic.
 In~Chapter~\ref{sc}, we characterise sequential consistency of shared memory in \iec.
 In~Chapter~\ref{wf}, we characterise waitfree communication under sequential
 consistency.
 Chapter~\ref{related} compares our work with others.
 In Chapter~\ref{conclusion}, we summarise our achievements.
 In Chapter~\ref{discuss}, we treat issues including informal speculation and future work.
...
\chapter{Intuitionistic Epistemic Logic}
\label{iec}
...
\section{Informal Explanation by BHK-Interpretation}
\label{bhk}
...
of BHK-interpretation given in Section~\ref{bhk}.
...
See Section~\ref{Shavit} for details.
...
compare the two relations $\vdash$ and $\models$ in the next sections.
...
The main originality of this section is the following definition of the function $f_a$.
...
\subsection{Properties of the Auxiliary Functions}
...
Actually, in the next section, a variant of this function $f_a$ will be used for
...
\section{Strong Completeness and Finite Model Property}

In this section, we show strong completeness and finite model property.
...
\begin{definition}
We modify $f_a$ introduced in the last section (Definition~\ref{gafa}) and define $f'_a$ as:
 \[
...
\section{Adding Double Negation Elimination}
...
\section{Conservativity over the Fragment without Disjunction}
...
The aim of this section is to prove the conservativity.
...
\section{Formula Translation from Classical S4}
\label{ftfcs}
...
$\mathbf{S4}$ and \iec.
In this section, we show a connection between classical multi modal $\mathbf{S4}$ logc and
...
\section{Comparison with Creative Subject Argument}

Dummet~\cite[Section 6.3]{dummett2000elements} gives an axiom candidate
\[
...
\chapter{Axiom Type for Sequential Consistency}
\label{sc}
...
In this section, we characterise sequential consistency with a set of axioms.
...
\section{Logical Representation of the Problem}
...
In Chapter~\ref{sc},
in this chapter, we express wait-freedom as a class of formulas.
...
\section{Representation of Schedules as Models}
...
\section{Decidability of Solvability of Waitfree Task Specification}
...
\section{Sequential Consistency or Linearizability}
...
\section{The Cost of Monotonic Reasoning: Latency versus Throughput}
...
\section{Disjunction Distribution Over $K$~Modality}
...
\section{Relationship with Intuitionistic Predicate Logic}
