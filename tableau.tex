\chapter{A Terminating Tableau System for Sequential Consistency Logic}
 ...
 We show this property in a generalized form.
  ...
  Let us assume that
  there are a countably infinite set~$\pvar$ of \index{propositional variables} and a
  finite set~$\agents$ of \index{agents}.
  ...
  \begin{definition}[Kripke frame]
   A \index{frame} $\tuple{W,\preceq,(f_a)_{a\in \agents}}$ is a tuple where
   \begin{itemize}
  ...
  \begin{definition}[Kripke model]
   A \index{model} $\tuple{W,\preceq, (f_a)_{a\in \agents},\rho}$ is a tuple where
   \begin{itemize}
  ...
  We assume that there is a countably infinite set of \index{world
  variables}~$\wvar$.
  We use $\mathsf v, \mathsf w,\ldots$ to denote world variables.
  We define \index{world terms} using BNF:
  \[
  ...
  of agents (a postfix).
  A \index{world inequality} is an inEquation of the form $\mathsf s\le
  ...
  \begin{definition}
   ...
   \end{itemize}
  \end{definition}
  A \index{world restriction set} is a finite set of world restrictions.

  A \index{world valuation} maps a world variable into a world of a frame.
  ...
  \begin{definition}
   We define the \index{satisfying relation} $M,w\models\varphi$ over a model
   ...
   \end{itemize}
  \end{definition}
  A model~$M$ \index{satisfies} $\varphi$ iff $M,w\models\varphi$ holds for any
  state~$w$ of~$M$.
  ...
  and Wallen's method for intuitionistic logic~\cite{waaler1999tableaux}.
   ...
   The deduction system\,\LB\,uses \index{prefixed formulae}.  A
   ...
   A \index{sequent} is shaped like
   ...
   \index{conforms to} world restriction set~$\mathsf R$ when all of the
   following hold:
   \begin{itemize}
  ...
  \begin{definition}
   A \index{refutation ladder} is a ladder made of the rules in
   ...
   \end{description}
  \end{definition}
  A \index{refutation ladder of a sequent}~$S$ is a refutation ladder
  with
  $S$ at the bottom.
  ...
  \begin{definition}
   A \index{complete refutation ladder} of~$S$ is a refutation ladder which is
   \begin{itemize}
  ...
  refutation ladder of $S$, and then there is a finite counter model.
   ...
   occurring in~$L$.
   We say $L$ is \index{unprovable} when $\cup L$ is.
