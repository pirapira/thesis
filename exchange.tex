...
shown in the Sync rule occurrence introducing the communicating term.
 ...
 \begin{theorem}[Type Preservation of Amida calculus]
  \label{safety}
...
\end{center}
  ...
  However, adding eval-subst rule breaks the current proof of
    ...
    \item[($\sendtype\psi{\phi^\sim}$)]
	 Using the induction hypothesis (IH.), we obtain a derivation:
	  \begin{center}
	   ...
	   x z t}{\one}$}
	   \UnaryInfC{$\tj{x}{\sendtype{\psi}{\phi^\sim}}\tr\tj{y}{\recvtype
...
\begin{theorem}[Completeness of Amida Calculus for Abelian
 Logic]
...
Amida calculus.
\begin{theorem}[Soundness of Amida Calculus for Abelian
 Logic]
...
\end{corollary}
\begin{corollary}[Excluded Middle in Amida calculus]
 The law of excluded middle $\phi\otimes(\phi\limp \one)$ is inhabited
 in Amida calculus.
\end{corollary}
\begin{corollary}[Prelinearity in Amida calculus]
...
\citet{giunti2010} gives a type system for the pi calculus with the type
...
Since the typing derivations of Amida calculus can be interpreted as
