...
We assume coutably infinitely many variables $x, y, z, \ldots$.
...
\subsection{Interaction with Contraction and Weakening}
 ...
 \begin{proof}
  ...
  response that is never obtained.
 \end{proof}
...
\section{Type Safety}
 ...
 \begin{proposition}[Safety]
  ...
  an evaluation sequence $t_0\eval v_0\hmid \cdots t_n\eval v_n$
  derivable, then
  \[
...
term, there is no closed canonical form ? satisfying the evaluation relation.
  ...
  \label{sec:session-process}
    ...
    opposite, but as Wadler~\citep{wadler2012propositions} explains, we are
...
\begin{proof}
 ...
 the Amida calculus rules satisfy the properties \textbf{mp} and
 $\with$\textbf{I}.
...
These axioms and rules are enough to prove that all inhabited types of
Amida calculus.
...
\end{corollary}
\fix{do it or delete this sentence.}
The terms of these types will be shown in Subsection~\ref{computational-meaning}.

\section{Proof Nets}
\label{sec:proofnets}

Toward evaluating all typed terms, a technique called proof nets seems
promising.
...
The most complicated example in \citet{pfenning2010} involves a drink server.
 \begin{example}[Drink Server from \citep{pfenning2010} in Amida
  ...
  and terminates.
  We can combine the server with either of these clients.
  However, since we lack the exponential modality, our type system
  cannot type any term with $!ServerProto$.  There we cannot beat
