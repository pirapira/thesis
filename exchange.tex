...
\texttt{putMVar} and \texttt{getMVar} commands.
...
Japanese schoolchildren use ``Amida lotteries''
...
\thref{thm:generalconvergence}), we have to evaluate the subterm~$s$ in
$\lambda
 ...
 \begin{proof}
  ...
  \begin{description}
   ...
   \item[($u$ is of other forms)]
	...
	$t\eval_\emptyset v\hmid u[v/x]$ stays in the same branch.
	For that we move any rule touching $t\eval_\emptyset v$ upwards
	\fix{how}
	and any branching rules upward.
...
$u\in\semo{\G\tr\tj t\phi}$.
 ...
 \end{proposition}
  ...
  \DisplayProof
	    ...
	    \fix{add this same-height condition in some} so
	    that we can determine $v$
    ...
    \begin{align*}
     ...
     \recvtype\phi\psi&\equiv \phi\otimes\psi &\text{inputting value of
     type $\phi$ then behave as $\psi$}\\
...
\fix{define correct Amida nets after defining correct essential nets}
...
\includegraphics[scale=0.4]{crossing.eps}
 \end{center}
in terms of reachability between endpoints\footnote{Differently from
...
One of their typing rules
 ...
 useful within the realm of reversible computation \fix{cite some
 denmark people}.
...
As a guidance we can take Dale Miller's $\mu$MALL~\fix{cite}.
