...
hypersequents reflect some computational intuition.
}.
...
x.t$ of type $\one\limp\phi$ so that $\lambda x.t\eval \lambda x.t$ is derivable.
  ...
  \label{sec:session-process}
    ...
    types\footnote{This is impossible using the ordinary linear types.}.
...
line from $p$ reaches.
The branching nodes labeled by $\parr^+, \parr^-, \otimes^+$ and
...
formulae so that the essential net is uniquely determined.}.
 ...
 the following correctness criterion.
  \begin{definition}[Correct essential nets]
   ...
   these conditions:
\begin{enumerate}
 \item Any node labeled with $X^+$ (resp. $Y^-$) is connected to a
       unique node labeled with $X^-$ (resp. $Y^+$).
       Any leaf labeled with $\bot^-$ is connected to a $\bot$-branch.
...
property so that we can confirm that the Amida axiom is not provable in IMLL.
 ...
 \begin{theorem}[Essential nets by~\citet{lamarche2008,murawski2003}]
  \label{essential-ok}
...
\end{itemize}
 \end{definition}
 In these clauses, we call the edges labeled~$e_a$ the \textit{Amida
...
\begin{enumerate}
 ...
 \item The end nodes of the translation correspond to the end nodes of
...
edges.
At the bottom, there is a layer that contains only ordinary
essential net nodes.
...
\citet{metcalfe2002} gave a labeled sequent calculus for Abelian logic.
Some years later,
...
Session types~\citep{interaction,honda-session} appeared in this chapter.
...
The most complicated example in \citet{pfenning2010} involves a drink server.
 \begin{example}[Drink server from \citet{pfenning2010} in the Amida
  cal.]
  \begin{align*}
  ...
  Here is the process of the server, which serves one client and
  terminates.\\
  \[
...
direction the path should continue; one plausible solution is allowing
multiple processes to track the paths in parallel and making these
processes synchronize on Amida edges.
