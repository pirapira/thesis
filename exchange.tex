...
We will see that the resulting logic is identical to Abelian logic~\citep{casari1989}.
...
$u\in\semo{\G\tr\tj t\phi}$.
 ...
 \end{proposition}
  ...
  \end{proof}
   ...
   Convergence holds because of subtle considerations of typing rules.
   One of those subtle considerations is
   the restriction that $\with$R rule only applies to singleton hypersequents.
   \begin{example}[The reason for restriction on $\with$R]
    There are no canonical terms $v$ such that $(c\ast) \otimes \lpair{\co
    c (\inl\ast), \co c (\inr \ast)} \eval v$ is derivable.
    By convergence, the term $ (c\ast) \otimes \lpair{\co
    c (\inl\ast), \co c (\inr \ast)} $ is not typable.
    Figure~\ref{fig:exchange:rules} allowed applying $\with$R rule to
    multi-component hypersequents, the term would be typable and
    thence, convergence would not hold.
   \end{example}
