...
With these rules, there is an s-term typed with the prelinearity axiom.
 ...
 \begin{center}
  ...
  \LL{Com}
  \bseq{}{\tj{\inl{\comodL}}{(X\limp Y)\oplus(Y\limp
  X)}\hmid\tr\tj{\inr{y}}{(X\limp Y)\oplus(Y\limp X)}}
  \LL{EC}
...
s-terms and s-stacks, the proof of adequacy (Theorem~\ref{c:adequacy}) gets
 ...
 we identify sequents and hypersequents up to exchange.
  ...
  \item[(Ax, Ax)] When both derivations consist of only axiom rules,
   ...
   \item[(IW, \textminus)]
	...
	\begin{enumerate}[label=\textit{(\arabic{*})}]
	 \item Take any
	       ...
	       By Proposition~\ref{prop:w}, $(t[\hat t/\hat x][\vec{g'}/\G'],
	...
	\begin{enumerate}[label=\textit{(\arabic{*})}]
	 ...
	 \item Variable~$x$ might be contained in $\G_0$, $\G_1$,
		...
		\item[(case $\G_0 = \tj{x}{\sigma},\hat\G_0$)]
		     ...
		     making $(t_1[\vec{g_1}/\G_1], {e_1\conc \eHo}\conc e'')$ in
		     $\sem{\phi'\limp\phi}$.
	       ...
	       \ref{c:first}.
		    ...
		    $\eHo\in\semo{\mathcal H_1}$,
                    ...
                    (
                     ...
                     [u[w/y][\vec{d}/\hat\D]\conc e'\conc {e_1\conc \eHo}\conc
                      f\conc f'']
                    )
                    \end{align*}
                    is in $\nsem{\phi_0\oplus\phi_1}$.
   ...
   \item[(Com, \textminus)]
	...
	\item
	     \fix{read through to check occurrences}
...
\begin{proof}
 ...
 By Proposition~\ref{prop:exec-on-pole}, this is a pole.
...
\begin{example}
 ...
 this s-term satisfies the condition of Proposition~\ref{spec:prelin}.
 By (dist),
 ...
 $\{(x,\pi_X), (y,\pi_Y)\}$ as stated by Proposition~\ref{spec:prelin}.
\end{example}
