...
They state that those tautologies are synchronizasion schemes.
Indeed in their formalism, reduction of one term can wait
for another, making synchronisation.
However,
...
We observe the computational behaviour of prelinearity axiom
...
one theread can give information to the other or
vice versa.
...
which is axiomatized by Dummet axiom $(\phi\imp\psi)\lor(\psi\imp\phi)$
...
\end{enumerate}
Condition~\ref{conc-closed}. is different from \citet{danos-krivine}'s
definition of poles.
  ...
  \item[(Ax, Ax)] When both derivations consist of only axiom rules,
	...
	\begin{enumerate}[label=\textit{(\arabic{*})}]
	 ...
	 \item Variable $x$ might be in $\G_0$ or $\G_1$.
		...
		\item[(case $\G_0 = \tj x\theta,\hat\G_0$)]
		     ...
		     $(\pi,e')\in\nsem{\phi}$ and
		     $(\sigma,f')\in\nsem\psi$.
		     We have to show tht the following executable is in
		     $\bbot$:
		     \begin{eqnarray*}
		     (\mat{t'}{\hat x}{t_0}{\hat
   ...
   \item[(Com, \textminus)]
	...
	\item
	     ...
	     We have to show that this executable is in $\bbot$:
	     \begin{align*}
	      & (t_0[\comodL/\hat x][\vec{g_0}/\hat\G_0][v/x]\conc
	      t_1[\comodR/\hat y][\vec{g_1}/\G_1]),\pi\conc \\
	      & u[w/y][\vec{d}/\hat\D],\sigma\conc e'\conc
	      {e_0}\conc {e_1}\conc  f\conc e''\conc f''
	     \end{align*}
	     \fix{where...}
	     This executable reduces to
	     \[
	     ...
	     because of Prop.~\fix{type of comods?}.
