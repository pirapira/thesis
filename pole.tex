...
concurrent elements of an executable.\fix{need a section for this}
...
\fix{describe danos and krivine}

\fix{describe what is interesting}

\fix{should say something about the asynchronous}

\begin{itemize}
 \item enabling us to what $X\supset X$ does.
 \item deeper understanding of asynchronity
\end{itemize}
...
An \textit{a-term}\index{term!a-term} (asynchronous term)~$t$ is an s-term or
$\reader c$.
...
\subsection{Statics}
For a set~$S$,
$\form(S)$ is the set of $S$-formulae~$\phi$:
\[
...
Hijek's \fix{spelling} basic fuzzy logic (BL) is very similar to MTL.
...
the additional axiom in order to find a lambda calcuus for BL.
