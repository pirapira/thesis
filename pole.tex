...
With these rules, there is an s-term typed with the prelinearity axiom.
 \begin{example}
  ...
  The s-term $(\inl{\comodL}\conc\inr{\comodR})$ has \fix{define have a
  type}
...
which can be traced back to \fix{fill in after looking at }.
The only deduction rule is modus ponens and the aximos are listed below:
\begin{description}
 \item[\MTLs 1] $(\phi\limp\psi)\limp(\psi\limp\chi)\limp(\phi\limp\chi)$
 \item[\MTLs 2] $\phi\limp\psi\limp\phi$
 \item[\MTLs 3] $(\phi\limp\psi\limp\chi)\limp(\psi\limp\phi\limp\chi)$
 \item[\MTLs 6] $((\phi\limp\psi)\limp\chi)\limp((\psi\limp\phi)\limp\chi)\limp\chi$
 ...
 \item[\MTLs 7]  $\bar 0\limp\phi$
 \item[\MTLs 4$\vee$a] $(\phi\limp\chi)\limp(\psi\limp\chi)\limp((\phi\oplus\psi)\limp\chi)$
 \item[\MTLs 4$\vee$b] $\phi\limp(\phi\oplus\psi)$
 \item[\MTLs 4$\vee$c] $(\phi\oplus\psi)\limp(\psi\oplus\phi)$.
\end{description}

\subsubsection{Soundness}

We translate each rule in our system into a single formula.

\begin{proposition}
 All formulae in \fix{what to what} is derivable in MTL.
\end{proposition}
\fix{prove}
