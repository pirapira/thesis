...
$c\neq \co c$ and
$\co{\co c} = c$.
A term\index{term}~$t$ is defined by BNF:
\[
 t::= x
 \mid (t)t
 \mid \lambda x.t
...
An executable\index{executable} is a multiset on $\Lambda\times\Pi$.
We use $\conc$ for delimiting elements in an executable.

% 2a' reduction
A schedule\index{schedule} is a total order on channels.
Given a fixed schedule~$\sche$, the reduction\index{reduction} relation~$\red$
on executables
...
A derivation\index{derivation} is a tree composed of the derivation rules below
...
An environment\index{environment} is a pair of a stack and an executable.
The set of environments is written as~$E$.
A program\index{program} is a pair of a term an executable.
...
The empty store\index{empty store}~$S_\epsilon$ maps any channel to $\bot$.
...
x)$ is $x$.
An executable\index{executable} is a finite multiset on $\Lambda \times \Pi$,
paired with a store.
