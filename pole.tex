...
There is a well-known substructural logic called the monoidal t-norm logic
...
\begin{corollary}[Prelinearity as a communication scheme]
 ...
 Assume that an a-term~$g$ has type $
		    {\forall X\forall
...
\begin{proposition}[Prelinearity as a communication scheme]
 \label{spec:prelin}
 Assume that an s-term $g$ has type $
{\forall X\forall
 Y((X\limp Y)\oplus (Y\limp X))}.
$
 Then, the s-executable
...
\begin{align*}
 ...
 X((\phi^\bullet\limp\psi^\bullet\limp X)\limp X) \text{ where }X\text{ does
...
\begin{proof}
 ...
 (EC) because in $\HLBCK$, the components in a hypersequent are distributed
...
shown\footnote{Although it is not clear how to implement shared memory
in the target process calculi.}.
