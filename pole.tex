...
An \textit{executable} is a multiset on $\Lambda\times\Pi$.
We use $\conc$ for delimiting elements in an executable.

% 2a' reduction
A \textit{schedule} is a total order on channels.
Given a fixed schedule~$\sche$, the \textit{reduction} relation~$\red$
on executables
