\chapter{Dummett's Axiom as an Asynchronous Communication Scheme}
...
concurrent threads exchange information asynchronously.

We observe the computational behavior of prelinearity axiom
$(\phi\limp\psi)\oplus(\psi\limp\phi)$ and left weakening
on top of the affine second-order implicational logic, which is the
fragment of second order
intuitionistic affine logic with connectives $\{\forall, \limp\}$
where $\limp$ is the affine implication.
We encode the disjunction~$\vee$ using the second order quantifier as
$\phi\oplus\psi\equiv \forall X((\phi\limp X)\limp (\psi\limp X)\limp X)$.
We interpret the disjunction~$\vee$ in the prelinearity axiom
...
Chapter~\fix{num}'s for G\"odel--Dummett logic,
which is axiomatized by Dummett axiom $(\phi\imp\psi)\lor(\psi\imp\phi)$
on top of intuitionistic logic.  \fix{encode and makw a better claim}
...
\fix{add a paragraph.  why synchronous.
store, effect, complicated.
}
...
$c\neq \co c$ and
$\co{\co c} = c$.
A \index{term}~$t$ is defined by BNF:
\[
 t::= x
 \mid (t)t
 \mid \lambda x.t
...
A \index{stack}~$\pi$ is defined by BNF:
\[
 \pi ::= \epsilon
 \mid t\cdot \pi
 \enspace.
...
An \index{executable} is a multiset on $\Lambda\times\Pi$.
We use $\conc$ for delimiting elements in an executable.

% 2a' reduction
A \index{schedule} is a total order on channels.
Given a fixed schedule~$\sche$, the \index{reduction} relation~$\red$
on executables
...
A \index{sequent} $\sequent{\G}{\tj t\phi}$ consists of a
context~$\G$ and a term~$t$ associated with a type~$\phi$.
The \index{context} is a finite sequence of variables associated with types.

A \index{derivation} is a tree composed of the derivation rules below
...
\subsection{Simulating Weakening and Disjunction}
...
So, we add left weakening as a rule:\\
\aseq{\G}{\tj t \phi}
\useq{\tj{x}{\psi},\G\tr\tj{t}{\phi}}
\DisplayProof

Here, we define $\phi\lor \psi$ as an abbreviation for
$\forall X((\phi\limp X)\limp(\psi\limp X)\limp X)$ for some~$X$ not
appearing in $\phi$ or $\psi$.

About this connective~$\vee$, these three rules are admissible:
...
The first is admissible because of the following partial derivation for
$X$ not appearing in $\G$ or $\phi$:\\
\aseq{\phi\limp X}{\phi\limp X}
\aseq{\G}{\phi}
\bseq{\phi\limp X, \G}{X}
\useq{\psi\limp X, \phi\limp X, \G}{X}
\useq{\phi\limp X, \G}{(\psi\limp X)\limp X}
\useq{\G}{(\phi\limp X)\limp (\psi\limp X)\limp X}
\useq{\G}{\forall X((\phi\limp X)\limp (\psi\limp X)\limp X)}
\DisplayProof

Moreover, $\vee$E is admissible because
of the following partial derivation:\\
\aseq{\G}{\forall X((\phi\limp X)\limp(\psi\limp X)\limp X)}
\useq{\G}{(\phi\limp\theta)\limp(\psi\limp\theta)\limp \theta}
\aseq{\phi,\D_0}{\theta}
\useq{\D_0}{\phi\limp\theta}
\bseq{\G,\D_0}{(\psi\limp\theta)\limp\theta}
\aseq{\psi,\D_1}{\theta}
\useq{\D_1}{\psi\limp\theta}
\bseq{\G,\D_0,\D_1}{\theta}
\DisplayProof

\fix{add: something like Dummett's axiom is provable}
...
A \index{pole}~$\bbot$ is a set of executables
which satisfies
\begin{enumerate}
...
An \index{environment} is a pair of a stack and an executable.
The set of environments is written as~$E$.
A \index{program} is a pair of a term an executable.
...
terms and stacks, the proof of adequacy (Proposition~\ref{c:adequacy}) does
  ...
  \item[(Ax, Ax)] When both derivations consist of only axiom rules,
   ...
   \item[(Com, \textminus)]
	...
	\item Take any $(\vec{g_0},{e_0})\in\sem{\G_0}$,
	      ...
	      By Proposition~\ref{squash}, we have $(t'_0\conc t'_1,
	...
	\item
	     ...
	     because of Proposition~\ref{comod-type}.
...
\begin{proof}
 ...
 By Proposition~\ref{prop:exec-on-pole},
 this is a pole.
...
\]
We write the set of terms as $\Lambda$ and stacks~$\Pi$.
A \index{store} is a partial mapping from locations to
terms, equivalently, a mapping from locations to terms and $\bot$'s.
The \index{empty store}~$S_\epsilon$ maps any channel to $\bot$.
...
x)$ is $x$.
An \index{executable} is a finite multiset on $\Lambda \times \Pi$,
paired with a store.
