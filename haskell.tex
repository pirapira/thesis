...
\subsection{Summary}
 ...
 Hypersequent calculus is introduced by mathematical
 ...
 for representing asynchronous communication.
...
waitfreedom provides no synchronization, it has
...
theorem.  We have developed lambda calculi in
Chapters~\ref{ch:exchange}, \ref{ch:lambda} and \ref{ch:pole}.
...
give input \texttt{apple}, the first thread tries to obtain the input for the other
...
\verb@ZeroT@ (it is a custom of Haskell community to use the same name
...
The value \verb@ZeroT@ is going to be used as the label for the 0th thread.
 \begin{code}
data ZeroT = ZeroT
...
\verb@ZeroT@, \verb@SucT ZeroT@, \verb@SucT (SucT ZeroT)@ and so on.
This technique is called type level natural numbers~\fix{cite someone}.
...
\verb@Done Finished@ shows that the thread has stopped and yielded a result.
\begin{code}
...
First we define a type class called \verb@HyperSequent@.
\begin{code}
...
\verb@l@ is of type class \verb@HyperSequent@, the type \verb@HCons e l@
...
termination.  Although the worker threads do not wait for any other
...
in the provided \verb@JobChannel@, 
\begin{code}
...
\verb@AbstractThreadId@ into a value of \verb@ThreadId@ and a MVar.
...
computation and then the main thread can see the worker thread has finihsed.
 \begin{code}
  type ThreadPool =
...
threadSpawn aid ch p = do
 ...
 \end{code}
 Then, the main thread waits for all threads and kills them.
\begin{code}
...
\verb@t -> IO a@ so that the computation can display the thread's
...
hypersequents\footnote{This implementation of permutations come from an
Agda~\fix{cite} implementation of a typed
lambda calculus for modal logic~K~\fix{cite}. Tatsuya Abe
presented the implementation to me in Tokyo.}.
...
\end{code}
Using this type class \verb@HLast@, we can define @cycling@ function
...
communication rule says two threads compute something, write it to
...
\end{code}
Here, both threads perform write and then read so that at least one
...
printThreeResults0 owner (c0, (c1, c2)) = putResult owner $
                      show $ [atid c0, atid c1, atid c2]

printThreeResults1 :: (Thread u, Thread s, Thread t, Thread v) =>
                      u -> ((s,t),v) -> IO ThreadStatus
printThreeResults1 owner ((c0, c1), c2) = putResult owner $
                      show $ [atid c0, atid c1, atid c2]
...
is implementable.  This at least supports the proof of
\thref{th:soundness}.
