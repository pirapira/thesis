...
\fix{Try proving Dummett's axiom in Begriffschrift}
...
implication $\varphi\supset\psi$ is a construction which permits us to
 transform any proof of $\varphi$ into a proof of $\psi$''~\fix{cite troelstra}.
The BHK
...
The formidable property is called subformula property\index{subformula property}.
...
Arthur Prior \fix{cite} invented a logical connective called tonk.
 \begin{center}
\AxiomC{$\phi$}
...
\fix{ML OCaml SML SML# Haskell F#}
...
dynamic epistemic logic \fix{cite} and inquisive logic \fix{cite}.  Thus
...
contradictory.  \fix{mention Yatabe}
...
propositional logic:
 ...
 Although additive implication can be defined \fix{cite troelstra},
 ...
 multiplicative implication
  \begin{center}
   \AxiomC{$\G,\phi\tr\psi$}
   \Understand{$\G\tr\phi\limp\psi$}
   \DisplayProof
   \hfill
   \AxiomC{$\G\tr\phi\limp\psi$}
   \AxiomC{$\D\tr\phi$}
   \BinaryInfC{$\G,\D\tr\psi$}
   \DisplayProof\enspace.
  \end{center}
...
which started with~\citet{girard1987} and \citet{danos-regnier}.
 ...
 \footnote{The Amida lottery has been known in Japan since \fix{?}} in a
 obfuscated way.
...
\begin{itemize}
 \item if your are interested in process calculi, \fix{}
 \item a proof net enthusiasist might find \fxi{} interesting
 \item shared memory consistency consistency,
\end{itemize}