...
property\index{subformula property}.
...
and dynamic behavior.  We can find examples in typed lambda calculi,
where static formalism of type derivations interacts with dynamic
behavior of lambda terms.
...
\]
 ...
 Henceforth, we do not give deduction systems of a logic and
...
logic\index{logic!G\"odel-Dummett} validates formulae
...
rule}\index{rule!communication}\index{communication rule},
...
In this thesis,
a \textit{logic}\index{logic} is a set of logical formulae that is
...
formulae.}.
Elements of a logic are called \textit{theorems}\index{theorem}.
The \textit{substitution}\index{substitution}-closedness means
 that if $\phi$ is a
...
G\"odel-Dummett logic has many proof searching implementations.
...
propositional logic:
 ...
 the rule is called \textit{invertible}\index{invertible}.} rule:
  \begin{center}
   \AxiomC{$\G,\phi\tr\psi$}
...
which started with~\citet{girard1987} and \citet{danos-regnier}.
 ...
 Japanese schoolchildren use the \textit{Amida lotteries}\index{Amida lotteries}
...
based on monoidal t-norm logic.  In this chapter, we analyze the
\textit{prelinearity
