...
To be specific, the term $\cotuple{M,N}$ reduces to $\inl{\inl{M}}$,
$\inl{\inr{N}}$ or $\inr{\gpair{M,N}}$ with $\mathsf{inl}$ and
...
\begin{itemize}
 ...
 \conf{}{\inl{\inl{M}}}$
 \item $\conf{}{\cotuple{M,N}}\areduce \conf{}{\inr{\gpair{M,N}}}$ where
...
to $\inl{\inr{M}}$, $\inl{\inl{N}}$ and $\inr{\gpair{N,M}}$.
To $\inl{\inr{M}}$, the term reduces as:
\begin{align*}
  (\emptyset, &\cotuple{(\comodL)M,(\comodR)N}) \\
 \wreduce (\{d\mapsto M\}, &\cotuple{\reader c,(\comodR)N}) \\
 \rreduce (\{d\mapsto M\}, &\cotuple{\abort,   (\comodR)N}) \\
 \areduce (\{d\mapsto M\}, &\inl{\inr{(\comodR)N}}) \\
 \wreduce (\{d\mapsto M, c\mapsto N\}, &\inl{\inr{\reader d}}) \\
 \rreduce (\{d\mapsto M, c\mapsto N\}, &\inl{\inr{M}})
\end{align*}
and symmetrically to $\inl{\inl{N}}$.
In the reduction to $\inr{\gpair{N,M}}$, the writes are followed by the reads:
\begin{align*}
 (\emptyset, &\cotuple{(\comodL)M,(\comodR)N})\\
 \wreduce (\{d\mapsto M\}, &\cotuple{\reader c, (\comodR)N}) \\
 ...
 \areduce (\{d\mapsto M, c\mapsto N\}, &\inr{\gpair{N,M}})\enspace.
\end{align*}
