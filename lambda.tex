...
First we define a lambda calculus and prove some of its properties.
Second we define typed waitfreedom.
Finally we prove that the lambda calculus characterizes typed waitfreedom.
...
$\varphi, \psi$ (also called local types\index{type!local}\index{local
type|see{type}}) by the following BNF,
where $I$ is a
...
A global formula\index{formula!global}\index{global
formula|see{formula}}
(also called a global type\index{type!global}\index{global type|see{type}})
is a non-empty partial mapping from processes to local
formulae.
...
\BinaryRule
 ...
 \\ where channels $c$ and $d$ are fresh.
\ruleskip
%% external structural
 \UnaryRule
 {$\hypert^+$}
...
To complete the definition of \lgd,
 ...
 \fix{define congruence}\\
 \fix{let configuration contain hyperterms}
...
In order to obtain the subformula property
 using proof normalization \fix{show subfmla or change here},
...
\reduce^\ast\cdots
$
with the following invariant \fix{what is $\mathcal M$?}:
...
\[
 ...
 l \leftarrow E; p
\]
where an expression\index{expression}~$E$ is
\begin{align*}
 E
 ::=\,\,
...
and every channel~$c$. is replaced by $v_c$.
If any of the substitutes is $\epsilon$,
$\semo{E}_{x, \vec v}$ is defined to be $\epsilon$.

A schedule\index{schedule} is an infinite sequence of nonempty subsets
of~$\processes$,
which looks like $\sigma = \sigma_0\sigma_1\sigma_2\cdots$.
We say $i$ is nonfaulty\index{nonfaulty} (resp.~faulty\index{faulty}) in
$\sigma$ if it appears infinitely (resp. only finitely many) often.
When every process is nonfaulty, the schedule is fair\index{fair}%
...
\begin{description}
 ...
 &(d\leftarrow \semoi{\hypert_0\hmid\overline{M}}(i); \p
 y\leftarrow c;)_j; \\
 &(c\leftarrow \semoi{\hypert_1\hmid\overline{N}}(i); \p
 x\leftarrow d;)_i\enspace,\\
 \semoi{\cotuple{\hypert_0,\hypert_1}\hmid (\comodL) M\hmid (\comodR) N} =&
 \semoi{\overline{\hypert_0}\hmid M} \cup \semoi{\overline{\hypert_1}\hmid N}
...
\begin{align*}
 ...
 \semoi{\hypert\hmid \linr{M}}=& \semoi{\overline \hypert\hmid M}\hmid
 \{{i, \p z}\}\enspace,
\end{align*}
...
&\llparenthesis
 ...
 \fix{define finished execution}\\
 \fix{define final shared memory snapshot, final system snapshot}

  \begin{proposition}
   \label{general-sound}
   For a typed hypersequent~$\hypert$, let $\semoi{\hypert} = f_0\hmid
   f_1\hmid\cdots \hmid f_n$ so that each $f_j$ is a partial mapping
   from processes in $\processes$ to program variables.
   Assume that there is a finished execution of $\semo{\hypert}$ whose final
   shared memory snapshot~$s$ satisfies
...
\[
\tj{x}{[0]\Id}, \tj{y}{[1]\Id}\tr
\tj{\{0\mapsto\cotuple{\{(\comodL)x,x\},\{x\}},
...
\fix{say, no problem with regard to provability 40min}
