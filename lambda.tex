...
A \textit{contexted type}
 ...
 $\Gamma$.
A \textit{hypersequent} is a finite sequence of sequents (each called
a \textit{component})
where the same
...
A \textit{typed configuration} is a \fix{add.. stores are empty}.
...
This property will be used in the proof of Thm.~\ref{first:sn}.
...
a typed protocol.
A \textit{program variable content} for $i\in\processes$ is a
partial
function
