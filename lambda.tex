...
processes: which process can take a step before which process does.
...
In other words, $\tr$ is interpreted as implication and $\hmid$ as
disjunction.
...
Let us assume a countably infinite set~$\pvar$ of propositional
variables \index{propositional
variable|see{variable}}\index{variable!propositional}.  We define local
...
A context\index{context} (denoted by $\Gamma$ and $\Delta$ possibly
...
An (extended) hypersequent\index{hypersequent} is a finite sequence of
...
A store\index{store} maps a location to a pure term or $\epsilon$,
where a pure term\index{pure term|see{term}}\index{term!pure} is a term
...
A binary relation~$R$ on configurations is a congruence\index{congruence}
when
\begin{itemize}
...
An investigator\index{investigator} is a partial map from processes
