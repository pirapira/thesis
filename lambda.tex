...
On the other hand, waitfreedom~\cite{Herlihy88,Saks:1993vq} is a class of distributed
...
Waitfreedom is a class of distributed computation
...
In this situation, computation is nondeterministic in general.
...
We assume a countably infinite number of \textit{processes}\index{process}.
...
We name such global formulae that map only one process as \textit{singleton
global formulae}\index{formula!singleton global}\index{global
formula!singleton}\index{singleton global formula}.
...
\caption[The underlying logic of \lgd.]
 ...
 In the names of the rules, $\intro$ at the end stands for introduction and $\elim$ for
 elimination.  For the structural rules, E stands for exchange, W for
...
is a global term.
In a sequent $\Gamma\tr\tj{\m M}{\m\varphi}$\kern -3pt, we require the
...
The hyperterm $(\m{M_i})_{0\le i \le n}$ is typable when the
...
store and $\mathcal O$ is a typable hypersequent.
...
\begin{itemize}
 ...
 \item $\concreteconf{S_0}{\hypert_0\hmid \m{M_0}} R
       ...
       $\concreteconf{S_0}{\hypert_0\hmid \m{M_0}\sqcup \m{N}} R
        \concreteconf{S_1}{\hypert_1\hmid \m{M_1}\sqcup \m{N}}$
 \item $\concreteconf{S_0}{\hypert_0\hmid \{i\mapsto M_0\}\sqcup
       \m{M_0}} R \concreteconf{S_1}{\hypert_1\hmid \imapsto{M_1}\sqcup
       \m{M_1}}$ implies\\
       $\concreteconf{S_0}{\hypert_0\hmid\imapsto{C[M_0]}\sqcup \m{M_0}} R
        \concreteconf{S_1}{\hypert_1\hmid\imapsto{C[M_1]}\sqcup \m{M_1}}$ for any
       context~$C$
...
\end{itemize}
Although only singleton terms appear in these clauses, by congruence,
these rules are applicable to more complicated global terms.
...
We want to avoid such synchronous communication.
...
Otherwise, in order to judge whether a configuration admits a write reduction or not,
we have to inspect the contents of the store.  This would make the
...
we add yet another kind of reduction rules called permutative
 reductions\footnote{The ``permutative'' conversions can be found in \citet{prawitz1965}.}.
...
Of a global type $\m\phi = (\phi_i)_{i \in I}$ (the global type that maps
$i$ to $\phi_i$),
...
\tj{x_2}{[j]\phi_2}, \ldots,\tj{x_n}{[j]\phi_n}$.
  ...
  \begin{proposition}[Process renaming]
  ...
  \begin{proof}
   ...
   with the modalities.  For the EC rule, we have to apply the induction
   hypothesis twice to change modalities in two components.
   For the com rule, let us assume the derivation ends in the com rule as:
   \[
   \BinaryRule
   {$\hypert_0\hmid\G\tr\tj{M}{[i]\varphi}$}
   {$\hypert_1\hmid\D\tr\tj{N}{[j]\psi}$}
   {}
   {$\hypert_0 \hmid \hypert_1 \hmid\Gamma\tr\tj{(\comodL)M}{[i]\psi}\hmid
...
.  This asymmetry allows us to encode waitfree
...
Using the original infinite sequence, we define a store called the
...
\begin{itemize}
 \item a read reduction $\conf{_k}{\hypc{}{\reader c}}
       ...
       \conf{_{k+1}}{\hypc'{O'}}$ for a context $\mathcal{C}'$.
...
strictly shortens the term under operation.
 ...
 the remaining difference from de Groote's lambda calculus~\citep{Philippe2002js}
 ...
 that consist of only these two kinds of reductions.
...
If a waitfree protocol solves an input-output problem, then we call the protocol
to be in the \textit{waitfreedom}\index{waitfreedom}.
We define the typed version of waitfreedom.
Since these definitions are involved, we supply many examples.
...
Program variables are typeset in the typewriter font.
...
A \textit{contexted type}\index{contexted type}
...
\end{itemize}
 \begin{example}[The addition protocol]
  ...
  (Example~\ref{ex:xor}).
  The set~$C$ of locations contains two elements $c$ and $d$.
  ...
  The last two sentences of each program write to the same variable.
  Although a variable is never updated after being filled, the second
  write is still meaningful because the first write may fail.
...
This definition is obtained by modifying Saks and Zaharoglou's
definition of input-output problems~\citep{Saks:1993vq}.
...
$\semo{E}_{x, \vec v}$ is defined to be $\epsilon$.
  \end{definition}
   ...
   \begin{definition}
    A \textit{block}\index{block} is a nonempty subset of $\processes$.
A \textit{schedule}\index{schedule} is an infinite sequence of blocks,
...
When every process is non-faulty, the schedule is \textit{fair}\index{fair}.
 ...
 \begin{example}[An example of schedules]
  ...
  then the schedule is fair.  In the other direction, a fair schedule
...
schedule~$\sigma$,
 ...
 \begin{example}[An example of a typed protocol solving the addition task]
  ...
  $\{0,1\}$'s.
  On the schedule~$\sigma$, both processes 0 and 1 are non-faulty.
  ...
  produces~$\epsilon$.
 \end{example}
  \begin{example}[A waitfreely unsolvable typed input-output
   problem]
...
\begin{definition}
 ...
 \textit{solvable}\index{solvable!--- by a global term} by a global term
...
essentially compilation from lambda terms to imperative
programs\footnote{That is one reason to try implementing it using
Haskell in Chapter~\ref{ch:haskell} because the Haskell compiler takes care of lambda-to-imperative translation.}.
...
This concludes the translation.
 \begin{example}[Translation of a typed term into a typed protocol]
  ...
  \end{align*}
  Next we look one step\footnote{A step in the type derivation is
  translated into a pair of equalities with different kinds of parentheses.} below:
  \begin{align*}
...
&= \left(d\leftarrow \p i_x; \p y\leftarrow c\right)_0;
 ...
 &\semoi{{\{0\mapsto add((\comodL)x,x)\}}\hmid
  \{1\mapsto add((\comodR)y,y)\}} \\
 &= \{0\mapsto \p z\} \hmid \{1\mapsto \p x\}\enspace.
\end{align*}
  \begin{align*}
&\llbracket
 {\{
...
&\llparenthesis
 {\{
  ...
  \end{align*}
  The last two clauses result in a typed protocol that solves the
  exclusive-or problem (Example~\ref{ex:xor}) for the same reason as the
  typed protocol in Example~\ref{ex:solv}.
 \end{example}

 Given that the translation solves a typed input-output problem, we have to
 ...
 We have to show that the translation is correct in the following sense.
  ...
  \end{proposition}
   \begin{proof}
    ...
    complicated cases are $ij$-com rule and the EC rule.
    \begin{description}
     \item[$ij$-com]
	  ...
	  By the induction hypothesis on $\hypert_0\hmid \imapsto{M}$,
	  ...
	  \processes$.
	  Also,
	  by the induction hypothesis on $\hypert_1\hmid \jmapsto{N}$,
	  there exists a reduction relation
	  \begin{align*}
...
\begin{proof}
\citet{Herlihy99} showed that a solution to a finite repetition
 ...
 elements.}.  Since \lgd\, can express problem composition,
...
Ciabattoni, Galatos and Terui~\cite{alg} give a class of logics
...
Nonetheless there is some more immediate future work.
...
Developing a second order formulation and performing the parametricity
argument is another piece of future work.
...
While doing that, we noticed that adequacy of parametricity argument
can be carried out using induction on just two type derivations;
if we did the same for the second-order variant of \lgd,\,it would require
induction over many derivations.

Another immediate task is to verify cut-elimination theorem for the new
communication rule presented here.
Our com rule does not duplicate any contexts
while the com' rule by Avron~\cite{avron91} duplicates
   the contexts $\G$ and $\D$ as
   \[
   \BinaryRule
   {$\hypert_0\hmid\G,\D\tr\tj{M}{[i]\varphi}$}
   {$\hypert_1\hmid\G,\D\tr\tj{N}{[j]\psi}$}
   ...
   \D\tr\tj{(\comodR)N}{[j]\varphi}$}\enspace.
   \]
Although \citet{avron91} showed cut-elimination theorem for com' rule,
it is not clear whether the same theorem holds in our case.

\section{Conclusions}
\label{conc}
We proposed \lgd, a lambda calculus
based on Avron's hypersequent calculus for
G\"odel-Dummett logic~\citep{avron91}.
