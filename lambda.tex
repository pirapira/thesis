...
We present the proof system using the hypersequent
style~\citep{avron91}.
...
Let us assume a countably infinite set of \index{propositional
variables}.
The set of propositional variables is written~$\pvar$.
We define \index{local formulae} $\varphi, \psi$ by the following BNF,
where $I$ is a
...
\]
Further, we define \index{global formulae} $\m\varphi, \m\psi$ with the following
BNF:
\[
...
[1],\ldots$ are called \index{modalities@modality}.
...
A \index{context} (denoted by $\Gamma$ and $\Delta$ possibly
subscripted) is a potentially empty
finite sequence of global formulae.
A \index{sequent}~$\Gamma\vdash\m\varphi$ is a pair of a context and a
global formula.
A \index{hypersequent} is a finite sequence of sequents.
...
\AxiomC{}
\LL{[0]Ax}
\UnaryInfC{$[0]P,[1]Q\vdash [0]P$}

\AxiomC{}
\LL{[1]Ax}
\UnaryInfC{$[0]P,[1]Q\vdash[1]Q$}
...
\LL{$\vee$I}
\UnaryInfC{$\vdash[0](P\supset Q) \lor [1](Q\supset P)\hmid \vdash[1](Q\supset P)$}
\LL{$\vee$I}
\UnaryInfC{$\vdash[0](P\supset Q) \lor [1](Q\supset P)\hmid
[0](P\supset Q) \lor [1](Q\supset P)$}
\LL{EC}
\UnaryInfC{$
\vdash [0](P\supset Q)\lor [1](Q\supset P) $}
\DisplayProof
...
We assume distinct, countably infinite sets of \index{variables},
\index{locations}
and
\index{processes}.
...
We define \index{terms}~$\term$ by the BNF
...
A \index{contexted type}
 $\Gamma\tr\m\varphi$ is a sequent without a term but with variables in
 $\Gamma$.
A \index{hypersequent} is a finite sequence of sequents (each called
a \index{component})
where the same
...
\fix{add terms to the following example}
\AxiomC{}
\LL{[0]Ax}
\UnaryInfC{$[0]P,[1]Q\vdash [0]P$}
...
$\Gamma\tr
\tj{M}{\m\varphi}$.
A \index{local term} is a term without $\overleftarrow l$,
$\overrightarrow l$ or any $\g$ constructs.
A \index{hyperterm}~$\hypert$ is a nonempty sequence of terms.
A \index{store} maps a location to a
...
A \index{configuration} is a triple $\conf{}{}\hypert$ of two
stores $\lstore, \rstore$ and a hyperterm~$\hypert$.
A \index{typed configuration} is a \fix{add.. stores are empty}.
...
\fix{state and prove subformula property somewhere}
...
A \index{redex} is a subterm that can be rewritten by a reduction.
A configuration~$\mathcal{C}$ is \index{normal} when there is no configuration
$\mathcal{D}$ with $\mathcal{C}\reduce \mathcal{D}$.
A term~$M$ is \index{normal} when the configuration $\conf{}{}{M}$ is
normal (the choice of $\lstore$ and $\rstore$ is irrelevant).

\fix{add reduction example from the term example and explain the
communicating behaviour}

\subsection{Properties}

An important property of
\lgd\, is \index{strong normalization}:
every typed hyperterm has a finite, maximal number of reductions it can
take.
Another is \index{non-abortfullness}: although some reductions yield
...
\end{center}
 ...
 by changing every modality to $[j]$. \fix{state and prove that this
 preserves deductions}
...
\end{theorem}
\begin{proof}
For proving this, we consider the \index{local fragment} that does not contain
...
\index{store prophecy} $(\lstore_\infty, \rstore_\infty)$ where
...
For $\abort$ propagation, we can apply the previous argument \fix{what?}.
\end{proof}
...
a \index{typed input-output problem} consists of each process's input type
...
A \index{program} is defined by BNF:
\[
 p ::= \epsilon\mid
 \p x\leftarrow E; p \mid
 l \leftarrow E; p
\]
where an \index{expression} is
\begin{align*}
 E
 ::=\,\,
...
A program is \index{well-formed} when
...
\begin{itemize}
 ...
 \item a typed program~$p_i$ for each $i\in \processes$,
       where
       a \index{typed program} is a well-formed program where all
...
a typed protocol.
A \index{program variable content} for $i\in\processes$ is a
partial
function
...
A \index{process snapshot} of $i\in\processes$ is a tuple
...
Process~$i$'s \index{output}~$\hat{o_k}$ at step~$k$ is
...
is \index{compatible with} $\vec d \in \prod_{i\in
...
is \linebreak[2] $R$-\index{permissible} iff there is at least one
vector $\vec d\in \prod_{i\in \processes}(\lval(o_i))$ with $(\vec x, \vec b)\in R$.
A typed protocol~$\Pi$ \index{solves} the typed input-output problem
...
An \index{investigator} $\tuple{i, \p x}$ is a pair of a process and a program
variable.
...
A \index{system snapshot satisfies a global formula}~$\m\varphi$
iff there exists a
finite set of
investigators~$I$ such that the system snapshot satisfies $I(\m\varphi)$.
A \index{system snapshot satisfies a context} iff it
satisfies every global formula in the context.
A \index{protocol} $p$ \textit{realizes a hypersequent}~$
\left(\Gamma_0\vdash
...
In the \index{participating set problem}~\cite{borowsky},
